...
class Trainer_gmn(object):
    ...
    def _forward_pass(self, batch, rollout=5):
        ...
        for _ in range(rollout):
            ...
            # x_input_1 = x_input_1 * (pi_all[:,:,0].unsqueeze(-1)) * self.rollout_mask[:,:,:2]
            # x_input_2 = x_input_2 * (pi_all[:,:,1].unsqueeze(-1)) * self.rollout_mask[:,:,:2]
            # x_input_3 = x_input_3 * (pi_all[:,:,2].unsqueeze(-1)) * self.rollout_mask[:,:,:2]
